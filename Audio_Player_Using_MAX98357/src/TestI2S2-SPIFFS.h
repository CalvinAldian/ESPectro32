...
void tryI2SRecord()
{
	...
	i2s_config_t i2s_config_rx = {
		...
		sample_rate : 128000,//sample_rate,
...
//        i2s_write_bytes(I2S_NUM_0, (const char*)buf, readable_bytes, portMAX_DELAY);
	...
	file = fs.open("/indonesia.wav", FILE_READ);
...
void tryI2SPlay()
{
	Serial.println("Opening file for playing");
	...
	File file = fs.open("/kodalin3.wav", FILE_READ);
	...
	i2s_set_sample_rates(I2S_NUM_0, 10000); //set sample rates
...
void tryWrite(){
	...
	File file = fs.open("/indonesia.wav", FILE_READ);
		...
		file = fs.open("/cek.txt", FILE_WRITE);
			if (!file) {
				ESP_LOGE(TAG2, "Failed to open file for writing");
				return;
			}
